...
<body>
			...
			<tbody>
				<c:forEach items="${cartItems}" var="item">
					<tr class="text-center">
						...
						<td class="col-5" id="gameAccountId" hidden>${item.gameAccount.accountId}</td>
		  ...
		  <div class="modal-dialog">
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		        ...
		        <p hidden><strong>Id:</strong> <span id="modalTotalAccountId"></span></p>
    ...
    $(document).ready(function() {
        ...
        $('#buyButton').click(function() {
            ...
            $('input[type="checkbox"]:checked').each(function() {
                ...
                let accountId = row.find('td:nth-child(5)').text(); // AccountId
            ...
            $('#modalTotalAccountId').text(accountIds.join(', '));
    ...
    const totalAccgameIdElement = document.getElementById("modalTotalAccountId");
    ...
    const accGameId = totalAccgameIdElement.innerText.trim();
    ...
    if (document.getElementById("publicKey").textContent.trim() === "") {
        $('#status_text').text("Chưa kết nối ví !");
    } else {
        await signInTransactionAndSendMoney(receiverAddress, quantity, accGameId);
    ...
    async function signInTransactionAndSendMoney(destPubkeyStr, quantity, accGameId) {
        (async () => {
            ...
            try {
                ...
                // Sau khi giao dịch thành công, gửi yêu cầu tạo hóa đơn
                let response = await $.post('/createOrder', {
				    totalAmount: quantity,
				    accountIds: accGameId
				});
				console.log("Tạo đơn hàng thành công:", response);

                // Chuyển hướng người dùng đến trang thành công
                console.log("Chuyển hướng đến trang thành công.");
        ...
        async function signAndSendTransaction(wallet, transaction, connection) {
            try {
		        ...
		        throw error; // Để tiếp tục xử lý lỗi ở các phần khác nếu cần
		    }
        }
    }
	</script>
